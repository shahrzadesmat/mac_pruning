...
torch.serialization.add_safe_globals([ConvNeXt, ResNet])
...
def parse_args():
    ...
    parser.add_argument('--is-finetuned', default=False, action='store_true', help='model is from finetuning script with complex checkpoint structure')
    ...
    parser.add_argument('--val-crop-size', default=224, type=int, help='crop size for validation')
    
    args = parser.parse_args()
    return args

def prepare_imagenet(imagenet_root, train_batch_size=64, val_batch_size=128, num_workers=4, use_imagenet_mean_std=True, interpolation='bicubic', val_resize=256, val_crop_size=224):
...
def main():
    ...
    # Prepare data loaders with val_crop_size parameter
    train_loader, val_loader = prepare_imagenet(
        args.data_path, 
    ...
    if os.path.isfile(args.model):
        ...
        if args.is_finetuned:
            # Handle finetuned model format
            if isinstance(state, dict):
                if 'model' in state:
                    if isinstance(state['model'], torch.nn.Module):
                        model = state['model']
                        print(f"[✅] Loaded model from checkpoint['model'] (nn.Module)")
                    else:
                        raise ValueError("Checkpoint contains state_dict but architecture unknown.")
                else:
                    raise ValueError("No model found in finetuned checkpoint.")
            else:
                model = state
                print(f"[✅] Loaded model directly")
        else:
            # Original loading logic for non-finetuned models
            if isinstance(state, dict):
                if 'model' in state:
                    model = state['model']
    ...
    # Count MACs and parameters using val_crop_size
    input_size = [3, args.val_crop_size, args.val_crop_size]
